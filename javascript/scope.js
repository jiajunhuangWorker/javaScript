...
当函数称为表达式时,它既不会提升,也不会污染全局对象,将函数变成一个函数表达式之一
如果书写一个函数表达式,然后立即调用称为立即执行函数 IIFE
闭包(closure)是一种现象,内部函数能使用外部环境中的变量，包括函数
*/
// 表达式 第一种方式 函数表达式  匿名函数
(function (){
	console.info(222)
})()
console.info('test' in window,"不污染全局变量,但是函数名不能在重复调用");
(function (){
	function num(){
		console.info('num')
	}
...
var obj ={
	name:"lu",
	say:function(){
		console.info('我是${this.name}'+this.name)
	}
}
obj.say()
// 第二种表达式
...
/*
	...
	执行环境是放到执行栈中的
	每个函数的调用都需要创建一个执行环境,函数调用结束执行环境销毁

	尾递归
	如果一个函数最后一条语句是调用函数,并且调用函数不是表达式的一部分,则称之为伪调用,如果是执行完毕后自身重新调用就叫尾调用
	某些语言或执行环境会对尾递归进行优化,他们会理解销毁当前函数,避免执行栈空间被占用
	在浏览器执行中V8尾调用没有优化在node 中有使用
	function f(n,total){
		if(n === 1){
			trturn total
		}
